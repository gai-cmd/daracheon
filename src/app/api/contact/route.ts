...
<p>대라천 고객지원팀 드림</p>`,
    ...
    if (adminEmail) {
      sendEmail({
        to: adminEmail,
        subject: `[대라천 관리자] 새 문의 접수 — ${validated.name} (${categoryLabel[validated.category] ?? validated.category})`,
        text: `새 문의가 접수되었습니다.\n\n이름: ${validated.name}\n이메일: ${validated.email}\n전화: ${validated.phone || '없음'}\n유형: ${categoryLabel[validated.category] ?? validated.category}\n내용:\n${validated.message}`,
