...
export async function POST() {
  try {
    ...
    console.error('[Admin TOTP] POST error:', error);
...
export async function PUT(request: Request) {
  try {
    ...
    console.error('[Admin TOTP] PUT error:', error);
    return NextResponse.json({ success: false, message: '서버 오류' }, { status: 500 });
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    // 레코드 삭제가 아니라 필드 제거(항목 수정)이므로 removedIds 불필요.
    await writeDataMerged('admin-users', users);
    ...
    console.error('[Admin TOTP] DELETE error:', error);
    return NextResponse.json({ success: false, message: '서버 오류' }, { status: 500 });
  }
}
