  ...
  return (
    <main className="mx-auto max-w-3xl px-5 pb-24 pt-28 text-[#fdfbf7]">
      <header className="mb-10 text-center">
        <p className="font-mono text-xs uppercase tracking-[0.28em] text-gold-400">ZOEL LIFE · 대라천</p>
        <h1 className="mt-3 text-3xl font-bold sm:text-4xl">제품 사용설명서</h1>
        <p className="mx-auto mt-3 max-w-xl text-lg leading-relaxed text-white/75">
          포장의 작은 글씨 대신, <b className="text-gold-300">복용 방법과 제품 정보</b>를 큰 글씨로 정리했습니다.
        </p>
      </header>
        ...
        {productGuides.map((g) => (
          <section key={g.slug} id={g.slug} className="scroll-mt-28">
            <div className="mb-6 flex flex-col items-center text-center">
              ...
              <h2 className="text-2xl font-bold sm:text-3xl">{g.name}</h2>
              {g.tagline && <p className="mt-2 text-lg text-gold-300">{g.tagline}</p>}
            </div>

            <div className="space-y-5">
              {g.sections.map((s) => (
                <div key={s.title} className="rounded-2xl border border-white/10 bg-white/[0.04] p-6">
                  <h3 className="mb-4 flex items-center gap-2 text-xl font-bold text-gold-400 sm:text-2xl">
                    <span className="inline-block h-5 w-1.5 rounded bg-gold-400" />
                    {s.title}
                  </h3>
                  <ul className="space-y-3">
                    {s.body.map((line, i) => (
                      <li key={i} className="flex gap-3 text-lg leading-relaxed text-white/90 sm:text-xl">
      ...
      <div className="mt-16 rounded-2xl border border-white/10 bg-white/[0.03] p-6 text-center">
        ...
        <Link href="/company#contact" className="mt-4 inline-block rounded-full bg-gold-400 px-7 py-3 text-lg font-bold text-[#1a1206] hover:bg-gold-300">
          문의하기
        </Link>
      </div>
    </main>
