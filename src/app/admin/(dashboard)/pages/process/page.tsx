                  ...
                  <div className="space-y-3">
                    ...
                    <div className="rounded-md border-2 border-amber-200 bg-amber-50/40 p-3">
                      ...
                      <div className="space-y-2">
                        {(ch.images ?? []).map((img, ii) => (
                          <div key={ii} className="flex items-start gap-2 rounded border border-gray-100 bg-gray-50 p-2">
                            ...
                            <div className="flex flex-col gap-1">
                              <button type="button" onClick={() => { const n = [...chapters]; n[ci] = { ...n[ci], images: moveItem(n[ci].images ?? [], ii, ii - 1) }; setChapters(n); }} className="rounded border border-gray-200 bg-white px-2 py-1 text-xs">▲</button>
                              <button type="button" onClick={() => { const n = [...chapters]; n[ci] = { ...n[ci], images: moveItem(n[ci].images ?? [], ii, ii + 1) }; setChapters(n); }} className="rounded border border-gray-200 bg-white px-2 py-1 text-xs">▼</button>
                              <button type="button" onClick={() => { const n = [...chapters]; n[ci] = { ...n[ci], images: removeIndex(n[ci].images ?? [], ii) }; setChapters(n); }} className="rounded border border-red-200 px-2 py-1 text-xs text-red-500 hover:bg-red-50">삭제</button>
                            </div>
                          </div>
                        ))}
                        <button
                          type="button"
                          onClick={() => { const n = [...chapters]; n[ci] = { ...n[ci], images: [...(n[ci].images ?? []), ''] }; setChapters(n); }}
                          className="rounded-md border border-dashed border-gray-300 px-3 py-1.5 text-xs text-gray-600 hover:border-gold-500 hover:text-gold-600"
                        >
                          + 이미지 추가
                        </button>
                      </div>
                      ...
                      <div className="mt-3 grid grid-cols-1 gap-2 md:grid-cols-2">
                        <input value={ch.imageAlt ?? ''} onChange={(e) => { const n = [...chapters]; n[ci] = { ...n[ci], imageAlt: e.target.value }; setChapters(n); }} placeholder="이미지 alt (공통)" className="rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-gold-500 focus:outline-none" />
                        <input value={ch.imageCaption ?? ''} onChange={(e) => { const n = [...chapters]; n[ci] = { ...n[ci], imageCaption: e.target.value }; setChapters(n); }} placeholder="이미지 caption (공통)" className="rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-gold-500 focus:outline-none" />
                      </div>
