...
# SPEC §D 는 SVG 내부 보존을 요구하지만, aria-label 의 '힐링 여행' 은 스크린리더가 읽는
# 건강 편익 프레이밍이라 컴플라이언스가 우선한다. 좌표·색·표시 텍스트는 전부 그대로 두고
# 접근성 설명 문구 한 곳만 중립 표현으로 바꾼다.
SVG_COURSE = SVG_COURSE.replace("침향 숲 힐링 여행 체험 코스 단계", "침향 숲 방문 체험 코스 단계")
assert "힐링" not in SVG_COURSE
...
A('<p>해변과 리조트만 보고 돌아가는 일정이라면 이 숲은 아예 시야에 들어오지 않습니다. '
  '침향나무가 자라는 자리를 직접 걷는 구간은 섬에서 흔한 코스가 아닙니다.</p>')
...
A('<p>기다리는 시간이 길다는 사실은 농장 운영 방식도 바꿔 놓습니다. '
  '묘목을 심은 사람과 원목을 거두는 사람이 같지 않을 수 있어, 나무마다 기록을 남겨 두지 않으면 이력이 끊깁니다.</p>')
A('<p>숲을 걸으며 굵기가 제각각인 나무를 지나치게 되는 것도 그래서입니다. '
  '한 해에 몰아 심은 조림지가 아니라, 시기를 나눠 심고 남겨 온 결과가 지금의 숲입니다.</p>')
...
A('<p>순서를 이렇게 잡은 데는 이유가 있습니다. 나무를 먼저 보고 향을 나중에 맡으면, 전시장에 놓인 오일이 어느 나무에서 왔는지가 함께 읽힙니다. '
  '반대로 향부터 맡고 숲으로 나가면 두 장면이 따로 남습니다.</p>')
...
A('<p>바로 옆에 선 어린 나무와 견주면 차이가 더 분명합니다. 줄기 지름과 껍질의 두께, 가지가 뻗은 폭이 눈으로 갈립니다. '
  '수령을 층으로 나눠 심은 농장이라 이런 비교가 한자리에서 가능합니다.</p>')
A('<p>도슨트에게 물어볼 것이 있다면 이 구간이 적당합니다. 나무를 앞에 두고 묻는 편이, 진열대 앞에서 묻는 것보다 답이 구체적으로 돌아옵니다.</p>')
...
A('<p>진열 순서에는 의도가 있습니다. 원목에서 시작해 완제품으로 닫는 배치라, 앞 진열대를 지나온 사람은 마지막 진열대의 제품이 어디서 왔는지를 이미 본 상태가 됩니다. '
  '중간 단계를 빼놓지 않는다는 점이 이 공간의 성격입니다.</p>')
...
A('<p>다만 향을 옮겨 적은 말은 사람마다 갈립니다. 짙다거나 묵직하다는 표현은 코로 받은 인상을 옮긴 것이지 성분 분석 결과가 아닙니다. '
  '대라천이 시향을 설명으로 대신하지 않고 자리를 따로 마련한 이유이기도 합니다.</p>')
A('<p>궁금한 점은 그 자리에서 도슨트에게 물어보면 됩니다. 한국어 통역 도슨트가 동행하면 통역을 따로 준비하지 않아도 됩니다.</p>')
...
A('<p>운영 시간은 연중무휴로 열려 있지만, 도슨트 동행과 통역은 사람이 붙는 일이라 미리 알려 주시는 편이 확실합니다. '
  '비용을 포함한 세부 조건은 이 글에서 확인해 드리지 못하므로 예약 시 문의해 주십시오.</p>')
...
A('<p>체험 농장과 생산 농장을 나눠 두면 관람 동선이 작업 구간과 겹치지 않습니다. '
  '방문객이 걷는 길과 나무를 관리하는 구역이 분리돼 있다는 뜻이기도 합니다.</p>')
A('<p>푸꾸옥에서 숲을 걷고 전시장에서 향으로 마무리하는 구성이 완결된 한 편으로 읽히는 것도 이런 배치 덕분입니다. '
  '섬 하나 안에서 나무부터 완제품까지 이어집니다.</p>')
...
out = {
    ...
    "excerpt": "조엘라이프(주)(브랜드 대라천)는 푸꾸옥 침향 숲 농장에서 100년이 넘은 고수령목과 50년생 나무를 함께 기릅니다. 숲길 코스와 전통 목조 직영 전시장의 시향·시음, 예약 방법, 대라천이 확인한 범위까지 차례로 정리했습니다.",
