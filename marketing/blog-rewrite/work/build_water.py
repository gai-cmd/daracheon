...
A('<p>규격집이 관능 규격에 침강을 넣어 둔 것도 이 관찰이 목질의 상태와 이어지기 때문입니다. 다만 규격집은 침강 하나만 적어 두지 않고 색과 맛을 함께 적었습니다. 한 항목만 떼어 보는 방식은 규격집의 서술과도 어긋납니다.</p>')
...
A('<p>관찰할 때는 조각을 넣고 잠시 기다립니다. 표면에 공기 방울이 붙어 있는 동안에는 뜨는 것처럼 보이기도 합니다. 조각의 크기와 물의 상태에 따라 장면이 달라지므로, 한 번의 관찰로 결론을 내리기보다 같은 조각을 여러 번 살펴보는 편이 낫습니다.</p>')
A('<p>이 시연이 답해 주는 질문은 하나입니다. 이 조각의 밀도가 물보다 큰가입니다. 그 밖의 질문, 곧 어떤 종인지 어디서 왔는지 어떤 등급인지는 물컵 밖에서 답을 찾아야 합니다.</p>')
...
A('<p>대라천은 이 세 가지 한계를 시연하는 자리에서 함께 설명합니다. 관찰의 범위를 밝히지 않은 시연은 근거가 아니라 인상만 남기기 때문입니다.</p>')
...
A(PAPERS_FIG)
A('<p>표의 세 줄은 각각 다른 문서로 확인됩니다. 한 장의 서류로 세 줄을 모두 채웠다고 설명한다면, 그 서류가 실제로 무엇을 증명하는 문서인지 되물어야 합니다.</p>')
...
A('<p>두 이름의 관계는 분류학 쪽에서 정리돼 있습니다. 식물분류학은 아갈로차를 말라센시스와 같은 종의 이명(異名)으로 다루고, '
  'CITES도 2010년 무렵 두 이름을 사실상 동일한 것으로 정리했습니다'
  '(<a href="https://cites.org" target="_blank" rel="noopener">CITES</a>). '
  '그러므로 학명 표기가 둘 중 어느 쪽이든 같은 종을 가리킬 수 있고, 표기 차이만으로 품질이 갈리지도 않습니다.</p>')
...
A('<p>대라천은 라벨과 검사서에 같은 학명을 적고 두 문서를 함께 제시합니다. 라벨에만 학명이 적혀 있고 대조할 검사서가 없다면, 그 표기는 확인되지 않은 상태로 남습니다.</p>')
...
A('{{IMG:farm-tree-tag}}')
A('<p>이력 확인은 질문 하나로 시작할 수 있습니다. 이 원료가 어느 농장에서 왔는지, 그 답을 뒷받침하는 기록이 있는지 물어보는 것입니다. 기록을 갖춘 곳은 번호로 답하고, 그렇지 않은 곳은 설명으로 답합니다.</p>')
A('<p>산지 확인이 물컵 시연과 갈리는 지점도 여기입니다. 침강은 지금 눈앞의 조각에 관한 관찰이고, 이력은 그 조각이 지나온 경로에 관한 기록입니다. 관찰은 한 장면으로 끝나지만 기록은 몇 번이고 되짚을 수 있습니다.</p>')
...
A('<p>확인한 것과 확인하지 않은 것을 나누어 적는 이유는 단순합니다. 검증 방법을 소개하는 글일수록 그 방법의 범위를 넘어선 결론이 붙기 쉽기 때문입니다. 대라천은 각 방법이 답해 주는 질문까지만 적고, 그 바깥은 적지 않습니다.</p>')
A('<p>대라천이 서류로 확인하는 범위는 원료의 품종과 산지, 이력, 검사 항목까지입니다. 그 원료가 개인의 몸에서 어떤 변화를 만드는지는 확인 대상이 아니며, 대라천은 그 부분을 이 글에서 설명하지 않습니다.</p>')
...
A('<p>출처는 세 갈래입니다. 대라천이 보유한 검사 성적서와 농장 기록, 식약처가 고시한 공식 규격, 그리고 외부 연구팀이 학술지에 발표한 리뷰 논문입니다. 앞의 둘은 대라천 원료에 관한 기록이고, 마지막 하나는 침향 일반에 관한 정리입니다. 원문 초안에 있던 출처 미상의 언론 보도 인용은 근거 문서를 확인할 수 없어 이번 판에서 제외했습니다.</p>')
A('<ul>'
...
out = {
    ...
    "changelog": {
        ...
        "voiceFixes": 0,
        "notes": "침강을 '진짜의 증거'로 읽히던 서술을 밀도 관찰로 한정하고, 품종·산지·등급을 증명하지 않는다는 한계를 전용 H2로 신설. 출처 없는 언론 보도·DNA 감별 동향 문장과 인도네시아 등재 배경 문단과 옛 문헌 산지 기록 주장은 검증 불가로 삭제. CITES 2010년 이명 정리 문장은 팀 리드 지시로 복원(cites.org 는 WebFetch 403 으로 본문 대조 불가, 발행 전 사람 확인 필요). -요체를 -습니다체로 통일하고 zoellife.com 절대경로를 내부 상대경로로 교체, A1·A7 인용과 수치 SVG·이미지 토큰 2종 추가."
    }
}
