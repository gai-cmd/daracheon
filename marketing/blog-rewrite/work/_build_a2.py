...
A('<p class="lead"><strong>결론부터.</strong> 조엘라이프(주)(브랜드 대라천)의 침향 제조 공정은 여섯 단계입니다. 20~30년 유기농 재배, 노니 발효액을 쓴 2~10년의 수지내림, 벌목 후 수작업 채취, 동나이 직영 공장의 전통 증기증류, 검사·인증, 캡슐화 순서로 이어집니다. 나무 한 그루가 채취 대상이 되기까지만 최소 22년이 걸리고, 길게는 40년입니다. 각 단계에서 대라천이 무엇을 확인했고 그 결과를 어떤 번호로 남겼는지, 검사번호와 인증번호를 붙여 단계별로 정리했습니다. 여섯 단계는 순서를 바꿀 수 없습니다.</p>')

A('<h2>침향 제조 공정은 어떤 순서로 이어집니까?</h2>')
A('<p>전체 흐름을 먼저 펼쳐 두면 각 단계가 왜 필요한지 읽기 쉬워집니다. 여섯 단계는 순서를 바꿀 수 없습니다. 앞 단계가 끝나야 뒤 단계의 조건이 만들어지기 때문입니다. 나무가 자라지 않으면 수지를 내릴 수 없고, 수지가 쌓이지 않으면 채취할 것이 없습니다.</p>')
A(FLOW)
A('<p>눈에 먼저 들어오는 것은 기간입니다. 대라천은 재배에 20~30년, 수지내림에 다시 2~10년을 씁니다. 두 단계를 더하면 나무 한 그루가 채취 대상이 되기까지 최소 22년, 길게는 40년이 걸립니다. 아래 그래프는 두 단계의 기간을 같은 축에 놓고 비교한 것입니다.</p>')
A(SVG)
...
A('<p>대라천은 이 원목을 DowGene DNA Testing에 유전자 검사로 의뢰해 아갈로차 유전자형과 100% 일치한다는 결과를 확인했습니다(검사번호 DA-260507-1). 품종을 겉모습으로 판정하지 않고 검사 결과로 고정한 것입니다. 재배 기간이 20년을 넘는 만큼, 시작 시점에 품종을 잘못 고르면 되돌릴 방법이 없습니다.</p>')
...
A('<p>조엘라이프(주)는 이 수지유도 기술로 특허 #12835를 확보했습니다(2014년 등록, 유효 20년). 2014년 노니 발효 시스템 도입과 2018년 수지유도제 개발이 그 실무 기록입니다. 발효액을 쓰는 방식은 유기농 인증 체계와 충돌하지 않아, 재배 단계의 인증을 그대로 이어 갈 수 있습니다.</p>')
A('<p>기간이 2년에서 10년까지 벌어지는 이유는 나무마다 반응 속도가 다르기 때문입니다. 대라천은 정해진 햇수를 채우는 방식이 아니라 수지가 충분히 쌓인 개체부터 다음 단계로 넘깁니다.</p>')
A('<p>이 단계에서 20년이라는 하한선을 두는 이유도 분명합니다. 어린 나무는 상처를 받아도 수지를 두껍게 쌓지 못합니다. 앞선 1단계의 재배 기간이 충분히 확보돼야 2단계가 성립합니다.</p>')
A('{{IMG:resin-inoculation}}')

A('<h2>3단계 — 왜 기계 대신 사람 손으로 골라냅니까?</h2>')
A('<p>수지가 충분히 쌓인 나무는 벌목한 뒤, 사람이 직접 손으로 수지가 밴 부분만 골라냅니다. 침향은 나무 전체가 아니라 수지가 스민 조직만을 가리키기 때문에, 벌목 자체보다 그다음 선별이 품질을 가릅니다.</p>')
...
A('<p>결과적으로 한 그루에서 나오는 침향의 양은 나무 부피에 비해 많지 않습니다. 대라천이 채취 단계에서 속도 대신 정확도를 택한 것은, 앞의 두 단계에 이미 22년 이상을 들였기 때문입니다. 마지막에 서두르면 그 시간이 회수되지 않습니다.</p>')
A(BLOB_FIG)
...
A('<p>대라천이 동나이를 증류 거점으로 삼은 근거는 토양과 물류입니다. 동나이는 미네랄이 풍부하고 배수·통기가 좋은 현무암 토양 지대이며, 호치민과 가까워 원료 관리와 출하를 함께 감당할 수 있습니다. 고지대의 서늘한 람동 농장은 연구개발과 오일 추출, 묘목 육성 거점으로 운영합니다. 증류 설비를 위탁하지 않고 직영으로 두면 원료 투입량과 산출량을 같은 장부에서 관리할 수 있습니다.</p>')
...
A('<p>검사를 통과한 오일은 마지막으로 캡슐에 충전됩니다. 완성된 제품에는 Lot 번호가 붙습니다. 앞선 다섯 단계가 원료를 다듬는 과정이었다면, 마지막 단계는 그 원료를 추적 가능한 형태로 닫는 과정입니다.</p>')
...
A('<p>대라천은 이 연결 기록을 사내 이력 자료로 관리하며, 원목 번호와 Lot 번호를 대조할 수 있게 보관합니다. 제품에 표시된 Lot 번호를 확인하는 방법은 대라천 공식 홈페이지에 안내돼 있습니다.</p>')
A('<p>캡슐이라는 형태를 고른 이유는 오일의 성질에 있습니다. 침향 오일은 공기와 빛에 오래 노출되면 향이 달라집니다. 캡슐은 정해진 용량을 밀봉해 그 노출을 줄이고, 낱개 단위로 Lot 번호를 붙일 수 있게 합니다. 오일캡슐은 GMP 기준으로 생산합니다.</p>')
...
A('<h2>근거·출처</h2>')
A('<p>공정 단계와 검사·인증 번호는 조엘라이프(주)의 공개 자료와 사내 문서에 근거합니다.</p>')
A('<ul>'
  ...
  '<li>Wang X, Chan SW, Singaram N 외 (2025), <em>Journal of Essential Oil Research</em> 37권 110~144쪽 — 추출법이 아퀼라리아 정유의 수율·조성에 미치는 영향 리뷰 — <a href="https://doi.org/10.1080/10412905.2024.2447706" target="_blank" rel="noopener">DOI</a></li>'
  '</ul>')
A('<p>대라천은 본문의 검사번호 DA-260507-1, 특허 #12835, TQC.19.1082-B·TQC.19.1082-A, TQC.05.1082, 68/QĐ-UBND, 2023-08-24 중금속 검사 결과, ISO·US FDA 등록번호를 원본 문서로 보유하고 있으며, 요청하시면 확인해 드립니다. 인용한 두 논문은 대라천이 수행한 연구가 아니라 해당 연구팀의 결과이며, 제품 효능을 뒷받침하는 자료가 아닙니다.</p>')
A(DISCLAIMER)
