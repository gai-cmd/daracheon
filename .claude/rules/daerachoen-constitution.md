...
> 이 파일은 `.claude/rules/`에 두며, Claude Code가 자동으로 컨텍스트에 로드한다.
> **규칙 층위 (2026-07-31 개정)**: 이 머신은 전역 `~/.claude/CLAUDE.md`(MoAI 지시서)와
> 프로젝트 `.claude/rules/moai/`(MoAI 작업 방법론)가 함께 로드된다. MoAI는 "어떻게 일하는가"
> (오케스트레이션·품질 게이트) 층이고, 이 헌법은 "이 프로젝트에서 무엇이 사실이고 무엇이 금지인가"
> (도메인 규칙) 층이다. **프로젝트 도메인 사항(배포·CSS·데이터·자산 정책)이 충돌하면 이 헌법이 우선한다.**
> (구판의 "이 파일이 단일 규칙 소스" 선언은 윈도우 시절 서술로, 현 macOS 환경과 맞지 않아 폐기.)

## 🖥 작업 환경 (macOS)

- 작업 디렉터리: `/Users/gai/personal/works/daerachoen` (git remote `gai-cmd/daracheon`).
...
- main 푸시는 곧 프로덕션 배포이므로 **푸시 전 반드시 사람 승인** (~/work 거버넌스 불변 규칙).
